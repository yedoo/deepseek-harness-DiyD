...
function renderThemes(snapshot: AppearanceSnapshot): string {
  ...
  const personal = snapshot.themes;
  return `<div class="page themes-page">
    <div class="toolbar"><div><h3>我的主题</h3><p>${active ? `当前使用：${escapeHtml(active.name)}` : "管理自己创建或导入的主题包。"}</p></div><div><button data-action="create-theme" class="primary">新建主题</button><button data-action="import-theme">导入主题包</button></div></div>
    <div class="section-heading"><h3>主题包</h3><span>${personal.length}</span></div>
...
function appearanceStyles(): string {
  return `
    ...
    .provider-panel { min-width:0; margin-top:10px; padding:13px; border:1px solid #e3e4e7; border-radius:10px; background:#fafafb; } .provider-line { display:grid; grid-template-columns:minmax(0,1fr) auto; } .provider-panel select { display:block; min-width:0; width:100%; max-width:100%; height:36px; padding:0 34px 0 10px; overflow:hidden; border:1px solid #dfe0e4; border-radius:8px; background:white; text-overflow:ellipsis; white-space:nowrap; } .provider-panel small { display:block; margin-top:7px; color:#8a8a93; } .provider-panel.error { color:#b42318; } .provider-actions { display:flex; gap:7px; margin-top:9px; } .provider-slider { display:grid; grid-template-columns:86px minmax(0,1fr) 44px; align-items:center; gap:8px; margin-top:10px; } .provider-slider input { min-width:0; width:100%; accent-color:#6b9df6; } .provider-slider output { color:#7a7a84; text-align:right; font-size:12px; }
    ...
    .toolbar { padding:4px 0 18px; } .toolbar button+button { margin-left:8px; } .theme-grid { display:grid; grid-template-columns:1fr 1fr; gap:12px; } .section-heading { margin:18px 0 10px; } .section-heading span,.count { color:#92929a; }
    ...
    .asset-grid { display:grid; grid-template-columns:repeat(3,1fr); gap:9px; margin-top:13px; } .asset-slot { min-height:78px; display:grid; grid-template-columns:32px minmax(0,1fr); grid-template-rows:min-content min-content; align-content:center; align-items:center; gap:3px 8px; padding-block:12px; text-align:left; } .asset-slot>span { grid-row:1/3; place-self:center; display:grid; place-items:center; width:30px; height:30px; border-radius:9px; background:#f0f2f6; } .asset-slot strong { align-self:end; line-height:1.35; } .asset-slot small { align-self:start; color:#8d8d96; } .asset-slot.selected { border-color:#98b7ef; } .danger-row { padding-top:18px; justify-content:flex-end; } .danger { color:#c43232; }
