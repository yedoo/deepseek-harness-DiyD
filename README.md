...
- Harness 可直接在应用内更新：staging 安装、版本校验、安全切换、健康检查与失败回滚
...
官方 Harness 使用独立版本通道：客户端读取当前运行版本，再与 npm 上的 `@deepseek-ai/dsh` 最新版本比较。用户确认后，新版及其依赖先安装到应用数据目录的 `harness-runtime/staging`，入口和版本校验通过后才切换到 `current`。客户端随后在新端口启动并执行健康检查；失败会恢复旧运行时或原源码安装。源码目录、`DSH_HOME` 与工作区文件都不会被更新器改写。
